...
function slideInNewPage(text, direction, direction2, animationTime, callbacks, url) {
	...
	newContent.waitForImages(function() {
        ...
        setPageMeta(newHTML); //in front of the perPageBindings because classes on body are needed for Qode Slider calculating position when left menu is transparent
        perPageBindings();
...
$j(document).ready(function() {
	...
	if($j('body').hasClass('woocommerce') || $j('body').hasClass('woocommerce-page')){	
		...
		$j(document).on('click','a[target!="_blank"]:not(.no_link)',function(click){
			...
			if($j(this).parent().hasClass('q_load_more')){ return false; }
			if($j(this).parent().hasClass('blog_load_more_button')){ return false; }
